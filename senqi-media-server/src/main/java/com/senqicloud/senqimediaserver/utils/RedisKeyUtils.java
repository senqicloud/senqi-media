...
public class RedisKeyUtils {
    ...
    public static String getEmailCaptchaKey(UserActionType userActionType, String email) {
        return String.format(
                "%s:%s:%s:%s:%s",
                RedisModuleKey.AUTH, RedisTypeKey.CAPTCHA, userActionType, RedisSceneKey.EMAIL, email);
    }
    ...
    public static String getSmsCaptchaKey(UserActionType userActionType, String phone) {
        return String.format(
                "%s:%s:%s:%s:%s",
                RedisModuleKey.AUTH, RedisTypeKey.CAPTCHA, userActionType, RedisSceneKey.SMS, phone);
    }

    // 图形验证码（比如验证码uuid）
    public static String getImageCaptchaKey(String uuid) {
        return String.format(
                "%s:%s:%s:%s", RedisModuleKey.AUTH, RedisTypeKey.CAPTCHA, RedisSceneKey.EMAIL, uuid);
    }
